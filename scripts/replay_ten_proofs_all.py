...
def _root_coverage(db_path: Path) -> dict[str, int]:
    coverage: dict[str, int] = {}
    with sqlite3.connect(db_path) as conn:
        for root in EXPECTED_ROOTS:
            count = conn.execute(
                "SELECT count(*) FROM nodes WHERE module = ? OR module LIKE ?",
                (root, f"{root}.%"),
            ).fetchone()[0]
            coverage[root] = int(count)
    missing = [root for root, count in coverage.items() if count == 0]
    if missing:
        raise AssertionError(f"full ten-proofs artifact missing root-module nodes: {missing}")
    return coverage
...
def run_replay(db_path: Path, artifact_path: Path, descriptor_path: Path) -> dict[str, object]:
    ...
    if manifest.scope.root_modules != EXPECTED_ROOTS:
        raise AssertionError(
            "full ten-proofs artifact root scope mismatch: "
            f"expected {EXPECTED_ROOTS}, observed {manifest.scope.root_modules}"
        )

    coverage = _root_coverage(db_path)

    exact_hits = search(db_path, TARGET, limit=1)
    if len(exact_hits) != 1 or exact_hits[0].source_path != "MulticolorTriangleRamsey.lean":
        raise AssertionError("erdos_183 did not resolve inside full ten-proofs artifact")
    ...
    if not all(str(edge["evidence_grade"]).startswith("ELABORATED_") for edge in graph.edges):
        raise AssertionError("full-corpus replay returned non-elaborated dependency evidence")
    ...
    if not any(hit.declaration_hint == "erdos_183" for hit in lexical):
        raise AssertionError("grounded full-corpus lexical query missed erdos_183")
