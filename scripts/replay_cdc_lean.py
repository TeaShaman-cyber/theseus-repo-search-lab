...
EXPECTED_DEPENDENCIES = {
    "lean:CDCLean.FiniteGraph.jaegerKilpatrickEightFlow",
    "lean:CDCLean.cycleDoubleCover_of_gammaFlow",
}
...
def run_replay(db_path: Path, artifact_path: Path, descriptor_path: Path) -> dict[str, object]:
    ...
    if len(exact_hits) != 1 or exact_hits[0].source_path != "CDCLean/Main.lean":
        raise AssertionError("cycleDoubleCover_of_bridgeless did not resolve to exact source provenance")

    graph = dependencies(db_path, TARGET, depth=1)
    if not graph.edges:
        raise AssertionError("cycleDoubleCover_of_bridgeless has no in-scope elaborated dependency edges")
    ...
    if not EXPECTED_DEPENDENCIES.issubset(direct_targets):
        ...
        raise AssertionError(f"CDC endpoint missing required direct dependencies: {missing}")

    lexical = search(db_path, LEXICAL_QUERY, limit=10)
    if not any(hit.declaration_hint == "cycleDoubleCover_of_bridgeless" for hit in lexical):
        raise AssertionError("grounded CDC lexical query missed cycleDoubleCover_of_bridgeless")

    ctx = context(db_path, TARGET, depth=1, token_budget=4000)
    if not any(chunk["declaration_hint"] == "cycleDoubleCover_of_bridgeless" for chunk in ctx["chunks"]):
        raise AssertionError("bounded context omitted CDC endpoint source")
