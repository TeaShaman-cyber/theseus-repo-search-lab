...
from scripts.replay_cdc_lean import run_replay
...
DESCRIPTOR = Path("producer/sources/openai-cdc-lean.json")
...
def write_fixture(root: Path, *, authoritative=True, producer=None, deps=("CDCLean.FiniteGraph.jaegerKilpatrickEightFlow", "CDCLean.cycleDoubleCover_of_gammaFlow")) -> Path:
    ...
    names = ["CDCLean.cycleDoubleCover_of_bridgeless", "CDCLean.FiniteGraph.jaegerKilpatrickEightFlow", "CDCLean.cycleDoubleCover_of_gammaFlow"]
    nodes = [
        node(names[0], "CDCLean.Main", source.source_commit),
        node(names[1], "CDCLean.JaegerKilpatrick", source.source_commit),
        node(names[2], "CDCLean.Main", source.source_commit),
    ]
    ...
    write_artifact(
        ...
        edges=edges,
        sources=[
            chunk("cycleDoubleCover_of_bridgeless", "theorem cycleDoubleCover_of_bridgeless : True := by trivial\n", "CDCLean/Main.lean", 50, source.source_commit),
            chunk("cycleDoubleCover_of_gammaFlow", "theorem cycleDoubleCover_of_gammaFlow : True := by trivial\n", "CDCLean/Main.lean", 35, source.source_commit),
            chunk("jaegerKilpatrickEightFlow", "theorem jaegerKilpatrickEightFlow : True := by trivial\n", "CDCLean/JaegerKilpatrick.lean", 1, source.source_commit),
        ],
...
class ReplayCdcLeanTests(unittest.TestCase):
    def test_valid_fixture_passes(self):
        with tempfile.TemporaryDirectory() as d:
            artifact, db = build_fixture(Path(d))
            result = run_replay(db, artifact, DESCRIPTOR)
            self.assertEqual(result["status"], "PASS")
            self.assertEqual(result["provenance"]["repo"], "openai/cdc-lean")
            self.assertEqual(result["exact"]["target"], "CDCLean.cycleDoubleCover_of_bridgeless")

    def test_rejects_incomplete_direct_dependencies(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            artifact = write_fixture(root, deps=("CDCLean.cycleDoubleCover_of_gammaFlow",))
    ...
    def test_rejects_wrong_runner_producer_pin(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            wrong = ProducerPin(kind="lean-dep-viz", tool_repo=RUNNER.extractor_repo, tool_commit="d" * 40, tool_hash=RUNNER.extractor_main_sha256)
