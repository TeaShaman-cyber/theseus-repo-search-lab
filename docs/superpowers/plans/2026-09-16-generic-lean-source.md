...
- [ ] **Step 3: Add fail-closed validation tests before implementation**
...
- [ ] **Step 4: Implement deterministic env mapping and CLI**
...
def environment_mapping(source, runner) -> dict[str, str]:
    return {
...
- [ ] **Step 6: Validate both committed descriptors through the production loader**
...
- [ ] **Step 7: Remove the mixed legacy producer file and run full suite**
...
- [ ] **Step 4: Resolve source root and observe the source-owned toolchain**

Use one shell expression that handles empty `SOURCE_SUBDIR`:

```bash
if [ -n "$SOURCE_SUBDIR" ]; then
  SOURCE_ROOT="$GITHUB_WORKSPACE/_target/source/$SOURCE_SUBDIR"
else
  SOURCE_ROOT="$GITHUB_WORKSPACE/_target/source"
fi
...
Do not compare this observed value to a descriptor copy; the exact Git commit is the authority binding the file.

- [ ] **Step 5: Keep one generic Lake build path**
...
- [ ] **Step 6: Keep one generic LeanDepViz extraction path**

Use the already pinned standalone extractor and pass all descriptor roots as one comma-separated value, matching LeanDepViz's parser contract:

```bash
python3 scripts/producer_guard.py run \
  --cwd "$SOURCE_ROOT" -- \
...
result = run_replay(db, source_root, artifact)
...
1. Add `from theseus_repo_search.artifact import load_artifact`.
2. Add `EXPECTED_REPO = "anthropics/formal-math"`.
3. Change the signature to `run_replay(db_path: Path, source_root: Path, artifact_path: Path)`.
4. Immediately inside the function add:

```python
manifest, _, _, _ = load_artifact(artifact_path)
if manifest.source_repo != EXPECTED_REPO:
    raise AssertionError(f"unexpected source repo: {manifest.source_repo}")
```
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_replay(args.db, args.source_root, args.artifact)
...
**Interfaces:**
- Consumes: one built SQLite projection, one source root, and one normalized artifact.
...
EXPECTED_REPO = "openai/LongGapsBetweenPrimes"


def run_replay(db_path: Path, artifact_path: Path) -> dict[str, object]:
    manifest, _, _, _ = load_artifact(artifact_path)
    if manifest.source_repo != EXPECTED_REPO:
        raise AssertionError(f"unexpected source repo: {manifest.source_repo}")
...
def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    result = run_replay(args.db, args.artifact)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(
        json.dumps(result, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
        encoding="utf-8",
...
Then run the corresponding replay against the fresh SQLite projection. No source build or Lean invocation is allowed in this consumer step.
